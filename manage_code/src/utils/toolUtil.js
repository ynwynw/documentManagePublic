...
const toolUtil = {
   ...
   */
  isMobile(s) {
    return /^(0|86|17951)?(13[0-9]|15[012356789]|16[6]|19[89]|17[01345678]|18[0-9]|14[579])[0-9]{8}$/.test(
      s
    )
  },
  /**
        ...
        if (menus[i].roleName == role) {
          if (menus[i].backMenu && menus[i].backMenu.length) {
            for (let j = 0; j < menus[i].backMenu.length; j++) {
              if (
                menus[i].backMenu[j].child &&
                menus[i].backMenu[j].child.length
              ) {
                for (let k = 0; k < menus[i].backMenu[j].child.length; k++) {
                  if (tableName.split('/').length > 1) {
                    if (
                      tableName.split('/')[0] ==
                        menus[i].backMenu[j].child[k].tableName &&
                      tableName.split('/')[1] ==
                        menus[i].backMenu[j].child[k].menuJump
                    ) {
                      let buttons =
                        menus[i].backMenu[j].child[k].buttons.join(',')
                      return buttons.indexOf(key) !== -1 || false
                    }
                  } else {
                    if (
                      tableName == 'orders' &&
                      menus[i].backMenu[j].child[k].tableName == 'orders' &&
                      !menus[i].backMenu[j].child[k].menuJump
                    ) {
                      let buttons =
                        menus[i].backMenu[j].child[k].buttons.join(',')
